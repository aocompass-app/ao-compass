...
/* --- 1. 4種類の日程それぞれに印が付く --- */
{
  ...
  ok(c[1].ev === 'エントリー開始' && c[1].pin === '🚩', `エントリー開始の日に印が付く (${c[1].pin}${c[1].ev})`);
  ok(c[2].ev === '出願締切' && c[2].pin === '📮', `出願締切の日に印が付く (${c[2].pin}${c[2].ev})`);
  ok(c[3].ev === '試験日' && c[3].pin === '✏️', `試験日の日に印が付く（今までは無印だった） (${c[3].pin}${c[3].ev})`);
...
/* --- 2. 重なった日は大事な順。別の大学どうしでも拾う --- */
{
  ...
  ok(c[5].pin === '✏️' && c[5].ev === '試験日,合格発表', `1校の中で重なっても大事な順 (${c[5].pin}${c[5].ev})`);
  await p.context().close();
}
